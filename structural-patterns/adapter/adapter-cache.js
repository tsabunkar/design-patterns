...
// creating hashcode generator -> which we give unique hashCode value
...
class LineToPointAdapter extends Array {
  constructor(line) {
    ...
    if (LineToPointAdapter.cache[this.hash]) return; // we already have it in cache
...
draw();
draw(); // !If we have grenerated points for the line, next time invoked we are using
// ! from-> cache rather than regenerating
